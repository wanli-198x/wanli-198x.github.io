  ...
  return __async(this, null, function* () {
    try {
      ...
      if (fs.existsSync(cachePath)) {
        const cachedHtml = yield fs.promises.readFile(cachePath, "utf-8");
        return cachedHtml;
      }
    } catch (error) {
    }
...
function setCachedHtml(config, filepath, content, html, url) {
  return __async(this, null, function* () {
    try {
      ...
      const cacheKey = getCacheKey(filepath, content, url);
      const cachePath = path.join(cacheDir, cacheKey);
      yield fs.promises.writeFile(cachePath, html, "utf-8");
    } catch (error) {
    }
  });
}
  ...
  const pluginOps = {
    ...
    configResolved(config) {
      return __async(this, null, function* () {
        ...
        yield cacheAllGitTimestamps(VPConfig.srcDir);
        const localesConfig = [];
